...
int8_t receiving_process(void)
{
  ...
  while(1)
  {
    cmd_type = protocol_frame_parse(frame_data, &frame_len);  //有可能是因为这个
//		set_p_i_d(&pid_speed, 10, 0, 0); 
//		set_pid_target(&pid_speed, 100);
    switch (cmd_type)
    {
      case CMD_NONE:
      {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        p_temp = *(float *)&temp0;
        i_temp = *(float *)&temp1;    //可以设置
        d_temp = *(float *)&temp2;
        
				/*要调那个PID就取消哪个注释*/
        //set_p_i_d(&pid_location2, p_temp, i_temp, d_temp);    // 设置那个改那个，只能通过通道观察
...
//				set_p_i_d(&pid_speed2, p_temp, i_temp, d_temp); 
      ...
      case SET_TARGET_CMD:
      {
				...
				/*要调那个PID就取消哪个注释*/
				//set_pid_target(&pid_location2, set_point);     //这个set_point值应该是脉冲数
...
//				set_motor2_disable();              // 停止电机
      ...
      case SET_PERIOD_CMD:  //这个功能不要了 
      {
        //uint32_t temp = COMPOUND_32BIT(&frame_data[13]);     // 周期数
        //__HAL_TIM_SET_AUTORELOAD(&htim,temp);                             // 设置定时器周期1~1000ms
				
				
				//野火的例程中，这个周期是中断调用PID计算函数的基本定时器的中断周期。
				//我是打算在在sysTick中调用PID的计算函数
      }
      break;
