...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)    //好像处理sysTick，其他定时器产生中断要调用回调函数时，都要进到这个函数里进行判断后在执行相应操作
{
	if(htim == (&htim7))  
	{
		...
		Motor_journey_cm	  = (  g_lMotorPulseSigma /(56*4*11) ) * (6.4*3.14);
		Motor2_journey_cm	= (  g_lMotor2PulseSigma /(56*4*11) ) * (6.4*3.14);
...
//		 }
		 /************/
			...
			{	
				 ...
				 if( (C1==0 && C2==0 && C3==0 && C4==0 && C5==0 && C6==0 && C7==0) || (C1==1 && C2==1 && C3==1 && C4==1 && C5==1 && C6==1 && C7==1) )
				 {
...
void Car_go_new()
{
	...
	g_fTargetJourney = 500;   //防止长时间PID控制用
	
	//如果巡线效果不好就将3.142加大
	Car_location   = ( 500 / (6.5 * 3.14) ) *   (56*4*11) ;    //Car_location 将  location_cm 转换为对应的脉冲数   g_fTargetJourney = 要转多少圈 * 1圈的脉冲数 
...
void spin_Turn(spin_dir_t zhuanxiang)   //传入小车的轮距(mm) 175mm  //其实转向环并不需要很精准，转弯后直接用直走时的巡线函数回正车身就好
{
	...
	/****转弯后不循线可以调转向系数**********/
		...
		{
			Car_Turn_val = ( spin90_val / (6.4 * 3.142) ) *   (56*4*11) ; 
...
float location_pid_control(void)  
{
	...
	int32_t actual_location;
	  ...
	  actual_location =  g_lMotorPulseSigma;   //1圈 = 2340个脉冲 = 56*4*11  //这里位置用圈数代替。
...
float location2_pid_control(void)  
{
	...
	int32_t actual_location;
	  ...
	  actual_location =  g_lMotor2PulseSigma;   //1圈 = 2340个脉冲 = 56*4*11  //这里位置用圈数代替。
